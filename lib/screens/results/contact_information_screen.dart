      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const _ContactTile(icon: Icons.phone_rounded, label: 'Primary Phone', value: '+92 --- -------', color: AppColors.primary, action: 'Call'),
            const _ContactTile(icon: Icons.phone_outlined, label: 'Secondary Phone', value: 'Not Listed', color: AppColors.textLight, action: null),
            const _ContactTile(icon: Icons.email_rounded, label: 'Email Address', value: 'hr@company.com', color: AppColors.secondary, action: 'Email'),
            const _ContactTile(icon: Icons.language_rounded, label: 'Website', value: 'www.company.com', color: AppColors.accent, action: 'Visit'),
            const _ContactTile(icon: Icons.location_on_rounded, label: 'Full Address', value: 'Building #, Street, Area,\nLahore, Punjab, Pakistan', color: AppColors.error, action: 'Map'),
            const SizedBox(height: 24),
            const Text('Social Media', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textDark)),
            const SizedBox(height: 14),
            const Row(children: [
              _SocialBtn(icon: Icons.link, label: 'LinkedIn', color: Color(0xFF0077B5)),
              SizedBox(width: 10),
              _SocialBtn(icon: Icons.code, label: 'GitHub', color: Color(0xFF333333)),
              SizedBox(width: 10),
              _SocialBtn(icon: Icons.facebook, label: 'Facebook', color: Color(0xFF1877F2)),
            ]),
            const SizedBox(height: 24),
